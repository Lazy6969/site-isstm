...
export default function Index({ memoires, filieresList, annees }) {
    ...
    return (
        <BiblioAdminLayout title="Mémoires & projets">
            ...
            <form onSubmit={submit} className="mb-8 rounded-2xl border border-slate-100 bg-white p-5 shadow-sm">
                <h2 className="mb-3 text-sm font-semibold text-isstm-navy">Ajouter un mémoire ou projet</h2>
                <div className="grid gap-3 sm:grid-cols-2">
                    ...
                    <input
                        ...
                        placeholder="Encadreur (facultatif)"
                        className="rounded-lg border border-slate-300 px-3.5 py-2 text-sm focus:border-isstm-navy focus:outline-none"
                    />
                    <select
            ...
            <div className="space-y-2">
                {memoires.map((m) => (
                    <div key={m.id} className="flex items-center gap-3 rounded-xl border border-slate-100 bg-white p-4 shadow-sm">
                        ...
                        <button onClick={() => destroy(m.id)} className="text-xs font-medium text-slate-400 hover:text-red-600">
                            Supprimer
                        </button>
                    </div>
                ))}
