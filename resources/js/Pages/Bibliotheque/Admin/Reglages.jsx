    ...
    return (
        <BiblioAdminLayout title={t('bibliotheque_admin.reglages_titre', 'Réglages — mentions, filières, années')}>
            ...
            <div className="grid gap-6 lg:grid-cols-3">
                <Card className="p-5">
                    <h2 className="mb-3 text-sm font-semibold text-isstm-navy dark:text-white">{t('bibliotheque_admin.mentions', 'Mentions')}</h2>
                    ...
                    <ul className="space-y-1.5 text-sm">
                        {mentions.map((m) => (
                            <li key={m.id} className="flex items-center justify-between gap-2">
                                <span className="truncate text-slate-700 dark:text-slate-200">{m.abreviation} — {m.nom}</span>
                                <button onClick={() => destroyMention(m.id)} className="flex-shrink-0 text-slate-400 dark:text-slate-500 hover:text-red-600" aria-label={t('communaute.supprimer', 'Supprimer')}>
                ...
                <Card className="p-5">
                    <h2 className="mb-3 text-sm font-semibold text-isstm-navy dark:text-white">{t('nav.filieres', 'Filières')}</h2>
                    ...
                    <ul className="space-y-1.5 text-sm">
                        {filieres.map((f) => (
                            <li key={f.id} className="flex items-center justify-between gap-2">
                                <span className="truncate text-slate-700 dark:text-slate-200">{f.niveau} - {f.nom} ({f.mention_abrev})</span>
                                <button onClick={() => destroyFiliere(f.id)} className="flex-shrink-0 text-slate-400 dark:text-slate-500 hover:text-red-600" aria-label={t('communaute.supprimer', 'Supprimer')}>
                                    <X className="h-3.5 w-3.5" aria-hidden="true" />
                                </button>
                            </li>
                        ))}
                ...
                <Card className="p-5">
                    <h2 className="mb-3 text-sm font-semibold text-isstm-navy dark:text-white">{t('bibliotheque_admin.annees_universitaires', 'Années universitaires')}</h2>
                    ...
                    <ul className="space-y-1.5 text-sm">
                        {annees.map((a) => (
                            <li key={a.id} className="flex items-center justify-between gap-2">
                                <span className="text-slate-700 dark:text-slate-200">{a.libelle}</span>
                                <button onClick={() => destroyAnnee(a.id)} className="text-slate-400 dark:text-slate-500 hover:text-red-600" aria-label={t('communaute.supprimer', 'Supprimer')}>
                                    <X className="h-3.5 w-3.5" aria-hidden="true" />
                                </button>
                            </li>
                        ))}
