...
export default function Reglages({ mentions, filieres, annees }) {
    ...
    function destroyMention(id) {
        if (confirm('Supprimer cette mention ? Les filières et mémoires rattachés seront aussi supprimés.')) {
            router.delete(`/bibliotheque/admin/reglages/mentions/${id}`, { preserveScroll: true });
        }
    }

    function destroyFiliere(id) {
        if (confirm('Supprimer cette filière ? Les mémoires rattachés seront aussi supprimés.')) {
            router.delete(`/bibliotheque/admin/reglages/filieres/${id}`, { preserveScroll: true });
        }
    }

    function destroyAnnee(id) {
        if (confirm('Supprimer cette année universitaire ?')) {
            router.delete(`/bibliotheque/admin/reglages/annees/${id}`, { preserveScroll: true });
        }
    }

    return (
        <BiblioAdminLayout title="Réglages — mentions, filières, années">
            <Head title="Bibliothèque — Réglages" />

            <div className="grid gap-6 lg:grid-cols-3">
                <section className="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm">
                    <h2 className="mb-3 text-sm font-semibold text-isstm-navy">Mentions</h2>
                    <form onSubmit={submitMention} className="mb-4 space-y-2">
                        ...
                        <input
                            ...
                            className="w-full rounded-lg border border-slate-300 px-3 py-1.5 text-sm focus:border-isstm-navy focus:outline-none"
                        />
                        <button disabled={mentionForm.processing} className="w-full rounded-lg bg-isstm-navy px-3 py-1.5 text-xs font-semibold text-white disabled:opacity-50">
                            Ajouter
                        </button>
                    </form>
                    <ul className="space-y-1.5 text-sm">
                        {mentions.map((m) => (
                            <li key={m.id} className="flex items-center justify-between gap-2">
                                ...
                                <button onClick={() => destroyMention(m.id)} className="flex-shrink-0 text-xs text-slate-400 hover:text-red-600">✕</button>
                            </li>
                        ))}
                    </ul>
                </section>
                ...
                <section className="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm">
                    <h2 className="mb-3 text-sm font-semibold text-isstm-navy">Filières</h2>
                    <form onSubmit={submitFiliere} className="mb-4 space-y-2">
                        ...
                        <button disabled={filiereForm.processing} className="w-full rounded-lg bg-isstm-navy px-3 py-1.5 text-xs font-semibold text-white disabled:opacity-50">
                            Ajouter
                        </button>
                    </form>
                    <ul className="space-y-1.5 text-sm">
                        {filieres.map((f) => (
                            <li key={f.id} className="flex items-center justify-between gap-2">
                                ...
                                <button onClick={() => destroyFiliere(f.id)} className="flex-shrink-0 text-xs text-slate-400 hover:text-red-600">✕</button>
                            </li>
                        ))}
                    </ul>
                </section>
                ...
                <section className="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm">
                    <h2 className="mb-3 text-sm font-semibold text-isstm-navy">Années universitaires</h2>
                    <form onSubmit={submitAnnee} className="mb-4 space-y-2">
                        ...
                        <button disabled={anneeForm.processing} className="w-full rounded-lg bg-isstm-navy px-3 py-1.5 text-xs font-semibold text-white disabled:opacity-50">
                            Ajouter
                        </button>
                    </form>
                    <ul className="space-y-1.5 text-sm">
                        {annees.map((a) => (
                            <li key={a.id} className="flex items-center justify-between gap-2">
                                ...
                                <button onClick={() => destroyAnnee(a.id)} className="text-xs text-slate-400 hover:text-red-600">✕</button>
                            </li>
                        ))}
                    </ul>
                </section>
